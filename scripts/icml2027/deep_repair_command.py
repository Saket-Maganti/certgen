#!/usr/bin/env python3
"""Run one CPU-only deep-repair command and append auditable ledgers."""
...
REPORT_ROOT = ROOT / "reports" / "icml2027" / "deep_repair"
LOG_ROOT = REPORT_ROOT / "command_logs"
CSV_LEDGER = REPORT_ROOT / "CERTGEN_DEEP_REPAIR_COMMAND_LEDGER.csv"
JSONL_LEDGER = REPORT_ROOT / "CERTGEN_DEEP_REPAIR_COMMAND_LEDGER.jsonl"
FIELDS = (
    "sequence", "phase", "command", "cwd", "start_utc", "end_utc",
    "duration_seconds", "exit_code", "status", "stdout_log", "stderr_log",
    "artifacts_created", "artifacts_reused", "blocker", "claim_allowed",
)
...
def _next_sequence() -> int:
    ...
    with CSV_LEDGER.open(encoding="utf-8", newline="") as handle:
        return 1 + max((int(row["sequence"]) for row in csv.DictReader(handle) if row.get("sequence")), default=0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    environment.update({
        "CUDA_VISIBLE_DEVICES": "", "CERTGEN_CPU_ONLY": "1", "PYTHONHASHSEED": "0",
        "OMP_NUM_THREADS": environment.get("OMP_NUM_THREADS", "1"),
    })
    started_utc = _utc_now()
    started = time.monotonic()
    result = subprocess.run(command, cwd=args.cwd, env=environment, capture_output=True, text=True, check=False)
    ...
    row: dict[str, object] = {
        "sequence": sequence, "phase": args.phase, "command": shlex.join(command), "cwd": recorded_cwd,
        "start_utc": started_utc, "end_utc": _utc_now(), "duration_seconds": f"{duration:.6f}",
        "exit_code": result.returncode, "status": status,
        "stdout_log": stdout_path.relative_to(ROOT).as_posix(), "stderr_log": stderr_path.relative_to(ROOT).as_posix(),
        "artifacts_created": args.artifacts_created, "artifacts_reused": args.artifacts_reused,
        "blocker": args.blocker, "claim_allowed": False,
    }
    _append(row)
