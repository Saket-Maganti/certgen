...
# V7 Prompt 10 — Multi-Benchmark Candidate Onramp

Do not run multi-benchmark experiments yet. Build a lightweight candidate onramp so scaling is ready after CIFAR-10 proves the path.

Create:

- `registry/provenance/multibench_candidate_sources.csv`;
- `docs/V7_MULTI_BENCHMARK_CANDIDATE_ONRAMP.md`;
- `python -m certgen.registry.validate_multibench_candidates`;

Candidate benchmarks:

- CIFAR-10;
- FFHQ/CelebA-HQ if released samples/checkpoints are easy;
- ImageNet-64/256 only as later scale;
- LSUN optional;
- video/FVD stretch only after image core works.
...
- expected compute;
- feature extractors;
- metric reproduction availability;
- risk;
- status.

Do not download or run. Do not create fake readiness.

This is a planning onramp, not evidence.
